...
class MainMenu extends AbstractMenu {
            ...
            switch (scanner.nextLine()) {
                ...
                case "2":
                    checkReservation(); // 예매 확인
                        return this; 
                case "3": 
                    canceReservation(); // 예매 취소
                    return this;

                case "4":
                    if (!checkAdminPassword()) { //관리자 비밀번호 확인
                        System.out.println(">> 비밀번호가 틀렸습니다.");
                        return this; //실패한 경우 메인 메뉴 객체반환(메인메뉴로 돌아간다.)
                    }
                ...
                case "q":
                    return null;
                default:
                    return this; // 그 외의 입력은 메뉴(this)로 돌아가게 함
                }
            }
    ...
    private void canceReservation() {
        ...
        try {
            ...
            String movieIdStr = scanner.nextLine(); //영화 코드를 찾아와야한다?
            Movie m = Movie.findByID(movieIdStr); // 예매할 영화 찾음
            ArrayList<Reservation> reservations = Reservation.findByMoviedID(movieIdStr);
            Seats seats = new Seats(reservations); //전구로 크레이티드 // 좌석 예약하기 메소드
            ...
            Reservation r = new Reservation(
                Long.parseLong(movieIdStr),// 영화대?표값 (영화 표값 같은데)
                m.getTitle(), // 영화 제목
                seatName // 좌석명
                );
                r.save();
