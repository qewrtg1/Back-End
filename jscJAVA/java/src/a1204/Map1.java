...
public class Map1 {
    ...
    Student s = readStudent(studentMap,2); //(1) 여기로 반환됨
    System.out.println(s!=null? s:"찾는 학생이 없다");
        ...
        private static Student  readStudent(Map<Integer,Student> studentMap, int key) {//(1) 이게
            // if( studentMap.containsKey(key)){
            //     return studentMap.get(key); //이게 요점임 중요함
            //   }
            //   return null; // 없으면 null을 반환한다. //좋은 방법이 맞으나 다른 방법도 해보겠음

            return studentMap.getOrDefault(key, null);
            //자바8이상에서만 사용가능한 명령어
            // getOrDefault : 주어진 key에 대응하는 값이 있으면 그 값(key)을 반환한다.
            // 없으면 defaultValue 를 반환한다.
