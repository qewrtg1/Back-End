...
//부모 클래스 (star craft)
// - 공통 메서드 life()를 정의. 자식 클래스들이 이를 오버라이드 한다.
// 자식 클래스 life() 메서드를 오버라이딩 하여 고유한 동작을 정의한다
// canon, run, fly와 같은 각 클래스만의 고유 메서드를 추가.


//메인 클래스

// 부모 클래스 배열 starcraft[]생성하여 다양한 자식 클래스를 저장한다
// 다형성을 활용해 life() 메서드를 호출하여 자식 클래스의 오버라이드된 메서드를 실행한다.
// instanceof 를 통해서 형변환을 사용하여 각 자식 클래스의 고유 메서드를 호출한다.
