...
public class PatientManager {
    private static ArrayList<Patient> patients; //환자 목록 //여기 페이션트를(1)
    ...
    public PatientManager(){
        patients = new ArrayList<>(); // 여기에 araylist를 주입 (1)
        patients.add(new Patient("홍길동", "010-1234-1234", "경기도 성남시", "치과", "12:30"));
        //더미데이터를 PatientManager 객체가 
        // 1개 생성(1)
...
//더미데이터에 입력이 되어는 있지만 더 해야한다?
//name부터 받아야 한다.
        System.out.println("========환자 등록=========");
        System.out.print("이름 : ");
        String name = sc.nextLine(); //next던 nextline이던 상관없음.
        sc.nextLine();
        ...
        patients.add(new Patient(name, phone, address, disease, time)); //disease: 질병이란 뜻이래

        System.out.println("접 수 중");
        System.out.println("\n" + name + "님 진료가 접수되었습니다.");
    }
        ...
        while(true){
            ...
            switch(choice){
                case 1:
                    ...
                    break;
                case 2:
                    cancelAppoint(); //환자등록 삭제  //여기까지. 이 이후는 몰라서 대충 붙여넣기 함
                        //사실 걍 여기 내용 전체를 반 이상 이해 못함.
                    break;
                case 3:
                    updatePatient(); //환자 등록 정보 수정
                        break;
                case 4:
                    return; // 메인메뉴로 돌아가게 한다고 함.
